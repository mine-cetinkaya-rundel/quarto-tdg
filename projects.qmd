...
You'll learn more about the `website` type in @sec-websites.
...
Since `.qmd` files are plain text, they are ideal for storing in a version control system like Git.
...
Behind the scenes, this button uses the rsconnect R package rather than `quarto publish`. If you use RStudio and are publishing to one of RPubs, Posit Connect or Posit Cloud we recommend reading the RStudio documentation on Publishing (<https://docs.posit.co/ide/user/ide/guide/publish/publishing.html>).
...
* To control computational dependencies for a project it is common to set up a computational environment. You can learn more in @sec-r and @sec-python.
