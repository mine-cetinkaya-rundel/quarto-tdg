...
Under `fancy`, `clouds` and `rainbow`, different types get different accent colors so you can tell a definition from a theorem at a glance.
...
For a document that passes validation, you'll see `[typst]` compile step completing with `DONE`, and a `PASSED` message from `veraPDF`:
...
To maintain sufficient contrast between your code and it's background, make sure you pick a dark-tuned syntax highlighting theme when you use a dark code background.

Also be aware, some syntax highlighting themes explicitly set a background color (e.g. `github`) and you can not alter it via brand.
:::

#### Fonts

To specify fonts, use `fonts` under `typography` to describe the `family` and `source`: `google`/`bunny` to fetch it at render time, or `system` for a font that's already installed.
...
`color` and `typography` are independent, so you can set both in one `_brand.yml` to style colors and fonts together, as in @fig-typst-brand-all.
...
You've seen Quarto provides you ways to customize appearance using document options, and via brand, but these touch a limited subset of what you can control.
...
A good place to start is the Typst tutorial followed by reading the [Syntax](https://typst.app/docs/reference/syntax/), [Styling](https://typst.app/docs/reference/styling/) and [Scripting](https://typst.app/docs/reference/scripting/) sections in the Typst Reference.
...
![A running header at the top of the page](images/typst-raw-typst-page-header.png){#fig-typst-raw-typst-page-header .border fig-alt="A PDF page with a Route Punctuality Report header at top, followed by Summary and Recommendations sections."}
...
At the finest level, any text, regardless of where it appears flow through the [`text()` function](https://typst.app/docs/reference/text/text/). 
The `text()` function has many arguments controlling the appearance of text like color (`stroke` and `fill`), font properties (`font`, `weight`, `style`, `size` and `stretch`) and many more.
...
These functions take the existing element as input and can return anything, but usually a transformed version of the input.
...
![Each heading followed by a thin gray horizontal rule](images/typst-raw-typst-heading-rule.png){#fig-typst-raw-typst-heading-rule .border fig-alt="Summary and Recommendations headings, each followed by a thin gray horizontal rule spanning the page width."}
...
When set and show rules aren't enough, you can customize further by providing your own **template partials**.
...
- You can provide your own definition for how the title, authors, and date etc. should be laid out.
...
As general best practice start by editing partials, and only if you can't get what you need override the complete template.
...
You saw how to make a PDF accessible by targeting the PDF/UA-1 standard, and how to customize its appearance through document options, brand, raw `set` and `show` rules, and, when those aren't enough, your own template partials.
