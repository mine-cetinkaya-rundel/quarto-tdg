...
Alt text makes an equation accessible to screen readers and is required to meet the PDF/UA-1 standard (you'll learn more about this standard in @sec-accessibility).
...
Highlighting themes you set in `syntax-highlighting` are designed for either a light or dark background. 
To maintain sufficient contrast between your code and its background, don't use a theme designed for a light background with a dark background, or vice versa. 

The themes `a11y`, `arrow`, `atom-one`, `ayu`, `breeze`, `github`, and `gruvbox` all have a dark variant, which you can select by appending `-dark` to the theme name.

Be aware that some syntax highlighting themes explicitly set a background color (e.g., `github`), and you cannot alter it via brand.
:::
...
This pulls in the `title` you've set in the document header, so changes to `title` will automatically propagate to our page header.
...
A good place to look for them is on [Quarto's format listing](https://quarto.org/docs/extensions/listing-formats.html) and the community-run [Quarto extensions listing](https://m.canouil.dev/quarto-extensions/).
