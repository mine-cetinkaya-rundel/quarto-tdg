...
Cross-references, shortcodes[^look-under-hood-2], and code annotation are examples of features that Quarto adds via filters.

[^look-under-hood-2]: Most shortcodes are implemented as filters, but there are two exceptions `include` and `embed`.
...
section headings are created with the TeX command `\subsection`, code cells use `Shaded` and `Highlighting` environments, and output uses appropriate commands like the `verbatim` environment for code output, and the `\includegraphics` command for the image file.
