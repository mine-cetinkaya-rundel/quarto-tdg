...
For `html-r.qmd`, HTML is the only format specified in the header, and assuming no `--to` argument was specified in `quarto render`, Quarto will know to target it.
...
Notice the document header, and Markdown content has passed through unchanged.
...
You'll see an example in @sec-pdf-r.
...
::::: {#fig-plot-md .column-body-outset layout-ncol="2"}
::: {#fig-plot-html}
``` markdown
...
The `quarto check` command is a good way to check the versions of these tools, and exactly which ones Quarto is finding.
...
-   **Quarto version**: Check your Quarto version against the latest release, particularly when documented features don't seem to work for you.
    Before reporting a bug, at minimum you should confirm your problem exists on the latest release, and it is good practice to also confirm it exists on the latest pre-release version.

-   **Internal dependencies**: Tools listed here are bundled with Quarto, and you cannot link Quarto to newer versions yourself.
    However, sometimes for trouble-shooting it can be useful to know these versions --- for instance, you might be trying to use a feature of Typst that is newer than the version bundled with Quarto.

-   **Quarto installation**: The version here will match the one reported on the first line, but adds the path to Quarto.
    If you manage multiple installations of Quarto, check you are getting the one you expect here.
...
-   **LaTeX**: Information about the LaTeX distribution Quarto will use.
...
-   **Python**: The Python 3 interpreter discovered by Quarto—check it is the Python you expect.
    ...
    If you are using a language other than Python with the `jupyter` engine, check the kernel you need is listed here.
...
The heuristics Quarto selects when selecting an engine are sometimes wrong.
...
If you want the `julia` engine, Quarto will never select it, you must opt-in via the `engine` key.
...
Trouble-shooting your environment is language specific and you should refer back to the relevant sections of @sec-r, @sec-python and @sec-julia.
...
When the engines process your code cells, they will include the label helping you to quickly identify which code cell caused the error.
...
You get output from `quarto render` it just doesn't look right.
...
* A decription of your problem, including what you expected to happen and what actually happened
