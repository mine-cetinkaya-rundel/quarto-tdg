...
Before anything else, Quarto first resolves any `include` shortcodes, then parses your file to extract the document header and code cells.
...
Quarto will also determine the set of formats that needs to be rendered. 
Each format is handled independently --- the document, along with the format specific metadata, travels through the computational engine and Pandoc independently of any other formats.

The combination of metadata and the language of code cells is used to select the appropriate computational engine.

### Computational engine

Each documemt is only ever run through one computational engine: Knitr, jupyter, or Julia.
...
The computational engine runs the executable code cells and replaces them with appropriate markdown, abiding by the `execute` options specified in the metadata, and options set in code cells.
...
That's because the engine is run once per format and you'll get an intermediate markdown file for each format specified in the metadata (or via command-line arguments).
...
Converting the Markdown to TeX commands isn't enough to have a complete TeX document. 
...
In contrast, for `format: typst`, Quarto runs the Typst version provided internally with Quarto. 
...
If you want to show an error in your document, set the option `error: true` in the corresponding code cell. This will prevent the error from halting execution, and include the error message in your rendered document.
...
What about the `./quarto` folder? Why is it automatically added to `.gitignore`?
