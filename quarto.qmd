...
Quarto documents and plain text files with the extension `.qmd` and they consist of three main components:

1. **YAML header:** Configuration metadata enclosed by `---` markers, specifying the title, format, and various options for your document.
...
2. **Code cells:** Executable blocks marked with three backticks  ` ``` ` and the name of the language in curly braces `{r}` containing your computational code,e.g.,
...
3. **Markdown text:** Narrative content with formatting, links, images, and other markdown elements.

Quarto documents can also include other elements such as figures and tables (which may be the result of executable code cells, created with markdown text, or included from an image), citations, cross-references, and more.
In later chapters we go into further detail about the various components of Quarto documents.
...
In the next chapter we go into further detail about using Quarto in various IDEs such as Positron, RStudio and a variety of others, including VS Code, Jupyter Lab, and others.
