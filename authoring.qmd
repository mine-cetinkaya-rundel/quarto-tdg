...
@tbl-inline-text-elements summarizes the most common things you want to put inside a paragraph or heading: formatted text, links, cross-references, citations, footnotes and inline equations.


Most 

We call the **inline text elements** because they are used *inside* block-level elements like paragraphs and headings.
You can also use them inside many of the other block-level elements discussed in this chapter like: in list items, block quotes, table cells, callouts, .

The rest of the chapter describes other block-level elements. 



We use the term **inline text** to refer to elements that can be used in the content part of a paragraph, heading or other block-level content (the rest of the chapter).


, such as bold or italic text, links, cross-references, citations, and inline equations.


Paragraphs and headings are examples of what we call **block** elements. 
Technically, you can use the syntax described in @tbl-inline-text-elements as the content in many block elements

anywhere you can have text, such as inside lists, block quotes


, or other block-level content.


@tbl-inline-text-elements summarize the syntax for common inline text elements in Quarto markdown.
...
### Text formatting {.unnumbered style="text-align: left"}

::: {.list-table header-rows=1 widths='2,1' aligns=l,l } 
...
Common syntax for inline text elements in Quarto markdown
:::
