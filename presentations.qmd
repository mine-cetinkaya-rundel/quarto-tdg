...
Same syntax can be used to create more than two columns, e.g., three columns with `width="33%"` each, or a mix of fixed and flexible widths, e.g., `width="200px"` for a fixed width column and `width="auto"` for a flexible column that fills remaining space.
...
RevealJS includes eleven built-in themes:
...
::: callout-note
We will use level 1 headings for section brakes and level 2 headings for slide breaks.
...
Note: Maybe a presenting section.
