...
When a slide contains a single top-level image, Quarto's `auto-stretch` feature automatically expands it to fill the remaining vertical space (see [Image size] for details on how this applies to computational figures). For manual control, use `.r-stretch` on an image to opt in to the same behavior:
...
By default, annotations appear as numbered markers that reveal explanations on hover or click. Use `code-annotations` to change this behavior:
...
As described in [Image sizing], Quarto's `auto-stretch` feature expands a single top-level figure to fill the remaining vertical space on a slide. This applies to computational output as well.
...
- **Slide tone** plays a subtle sound when you change slides---a helpful auditory cue, particularly for presenters who are blind or low-vision. Enable it with `slide-tone: true`.

- **Auto-slide** advances slides on a timer without any user input. Set `auto-slide` to a value in milliseconds (e.g. `auto-slide: 5000` for 5 seconds). Add `loop: true` to repeat the presentation continuously. A play/pause control appears automatically; press `A` to toggle it.

- **Slide zoom** lets you zoom into any element on a slide by holding `Alt` (or `Ctrl`) and clicking on it. Click again to zoom back out.
...
To include speaker notes in the PDF, append `?print-pdf&showNotes=true` to the URL before printing. Alternatively, use `?print-pdf&showNotes=separate-page` to place notes on their own pages after each slide.

::: callout-note
The `?print-pdf` view uses a different stylesheet optimized for print. Some visual effects like transitions and animations won't appear in the PDF.
:::
...
Accessible presentations ensure all audience members can engage with your content.

### Structure and semantics

- Use heading levels consistently—they define the slide structure for screen readers
- Keep slide content concise; dense slides are harder to follow for everyone
- Ensure a logical reading order by placing content in the order it should be read
...
```markdown
![A bar chart showing sales growth from 2020 to 2024](chart.png)
```
...
- Maintain sufficient contrast between text and background (WCAG recommends a minimum ratio of 4.5:1 for normal text)
- Don't rely on color alone to convey meaning—pair color with text labels, patterns, or icons
- Test your theme with a contrast checker before presenting

### Navigation aids

Enable features that help audiences follow along:
...
Setting `hash-type: number` creates predictable URLs (`#/3` for slide 3), making it easier for attendees to share or return to specific slides.

### Slide tone

For presenters who are blind or low-vision, enable auditory feedback when slides change:
...
- Navigate your presentation using only the keyboard (arrow keys, Tab, Enter)
- Use a screen reader to verify slide content is announced correctly
- Check that all interactive elements (links, tabs, buttons) are focusable and labeled
