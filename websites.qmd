...
This could be a local webserver like the one used for `quarto preview`, or a remote one like those provided by Posit Connect Cloud or GitHub Pages.
...
Working on a Quarto website is mostly like working on a single Quarto document.
...
In this section, you will learn how your project structure translates to your website structure.
You'll start by understanding how the location of your files in your project translates to the location of your pages in your site.
...
When you publish your website, the base URL will depend on where you host it. For GitHub Pages it might look something like `https://username.github.io/repository-name`, and on Posit Connect Cloud something like `https://username-project.share.connect.posit.cloud`, or you might even have a custom domain `https://yourdomain.com`.
...
You can link to identifiers on other pages, too, 
by adding them to the document path.
...
If you must have across page cross-references, consider using a `book` project instead.
...
In the rest of this section, you'll learn how to specify navigation items more flexibly and then see some more comprehensive examples of top and side navigation. 
...
| `icon`       | Name of one of the standard [Bootstrap 5 icons](https://icons.getbootstrap.com/) (e.g., `github`, `house`, `share`, etc.).                   |
...
You can specify the properties `light` and `dark` to `theme` to add a light/dark toggle (Figure @fig-website-theme-dark):
...
## Choose `light` and `dark` themes that are similar except for color

It's a good idea to choose themes that are as similar as possible in fonts, padding and other visual elements.
This ensures minimal visual disruption as people toggle themes. 
Another option to maintain visual consistency, is to use a single theme, but apply a light and dark brand (see @sec-brand-theme).
...
![`lux` used as the light theme](images/website-theme-light.png){fig-alt="Screenshot of webpage with a black navbar, white main body and dark text."}
...
To specify a light and dark theme, and add a light/dark toggle to your site, 
you can specify two brand files in your website configuration:
...
Where `_brand.yml` might, as it does above, define a dark mode with a charcoal background and mint text, 
and `_brand-light.yml`  defines a light mode with a mint background with charcoal text:

```{.yaml filename="_brand-light.yml"}
{{< include _examples/websites/brand-dark/_brand-light.yml >}}
```

### Combine **brand.yml** with a built-in theme {#sec-brand-theme}
...
If you have a light and dark brand, you can also layer them with a built-in theme.
In your website configuration, you'll specify your brand file under the `light` and `dark` properties for `brand`, and their layering with a built-in theme in the `light` and `dark` properties of `theme`:

```{.yaml filename="_quarto.yml"}
{{< include _examples/websites/brand-dark-theme/_quarto-layer.yml >}}
```

When you layer `light` and `dark` brands with built-in themes, you'll likely find a number of places the colors don't quite work. 
In that case, you'll likely need to make some color tweaks with CSS/SCSS.
...
However, you need to put `styles.scss` in `theme` first:
...
As an example, Bootstrap includes a [button component](https://getbootstrap.com/docs/5.3/components/buttons/):
...
In this section, you'll first learn about website metadata you'll likely want to set before publishing that will make it easier for people to find and share your website. 
Then, you'll learn about the most common approaches to publishing your site: the `quarto publish` command or hosting the `_site` folder.
...
You saw how to use `quarto publish` to publish a single document in @sec-sharing.
...
The `quarto publish` command supports several publishing providers.
The three we think are most useful if you are choosing for yourself are Posit Connect Cloud, GitHub Pages, and Netlify.
A brief summary comparing their features is shown in @tbl-providers.
...
If you'd prefer your source documents weren't available publicly you'll either need to upgrade your GitHub plan, or use something like Netlify or Posit Connect Cloud instead.
...
These features require a more complicated publishing setup you'll learn about in @sec-ci.
...
[Hugging Face](https://huggingface.co) has a more complicated setup and probably only makes sense if you are using Hugging Face beyond Quarto already.
...
This approach requires using `freeze` to store the results of the code execution.
...
Three common ways you might customize this workflow are as follows:
...
In this chapter, you learned about the key components of a Quarto website: content, navigation, and appearance. 
You also learned about a few techniques for publishing your site.

But, there are many other features we haven't talked about.
...
* Comments: Integrate third-party tools to allow your readers to leave comments on your pages, <https://quarto.org/docs/output-formats/html-basics.html#commenting>.
