...
Two special kinds of pages: About and Listings.
...
![Nested items in `navbar` using a dropdown menu](images/website-navbar-nested.png){#fig-navbar-nested fig-alt="Screenshot of a navigation bar with an item with the text 'People' which is expanded into a dropdown menu with the items 'Staff' and 'Students'."}
...
@fig-sidebar-simple that includes a logo, search box, and navigation links arranged as content and tools.
...
![](images/website-sidebar-auto.png){#fig-sidebar-auto fig-alt="A sidebar with links to: Blog; Faculty and Students nested under People; Past and Current nested under Projects; and Air Quality and Wastewater Monitoring nested under Current."}
...
I can't tell if this is really fully supported. It certainly doesn't look like what I thought was true is.
...
-    The first sidebar without a `title` or an `id`.
...
-   If all the sidebar elements have a `title` or `id`, the sidebar in which a link to the document is a navigation item, or no side bar at all.

-   If **not** all the sidebar elements have a `title` or `id`, the first sidebar without `title` or an `id`. 
...
The pages `project-1.qmd` and `project-2.qmd` will get the second sidebar containing links to each other.
Similarly, `course-1.qmd` and `course-2.qmd` will get the third sidebar containing links to each other.
The home page (`index.qmd`), about page (`about.qmd`), will get the first sidebar.
Any other page in the site will also get the first sidebar (with link to Home and About), unless they explicitly set another in their header.
...
An example of hybrid navigation dropdown versus not.

Read about how to implement Hybrid navigation at https://quarto.org/docs/websites/website-navigation.html#hybrid-navigation
...
| Element                                                                                            |                                                                                                                                                            | Option                                                                               |
+:===================================================================================================+:===========================================================================================================================================================+:=====================================================================================+
| [Top navigation](https://quarto.org/docs/websites/website-navigation.html#top-navigation)\         |                                                                                                                                                            | `navbar`:                                                                            |
...
+----------------------------------------------------------------------------------------------------+------------------------------------------------------------------------------------------------------------------------------------------------------------+--------------------------------------------------------------------------------------+
| [Side navigation](https://quarto.org/docs/websites/website-navigation.html#side-navigation)\       |                                                                                                                                                            | `sidebar`:                                                                           |
...
As you can see in @fig  `primary` is used in the top navigation bar and for links.
...
If you do alter `background`, and your pages include code blocks, you'll generally also want to change the colors used for code with the `typography` settings for `monospace-inline` and `monospace-block`:
...
Here's an example setting the main font (`base`), the font used for headings of any level (`headings`), and the font used for code (`monospace`):

```{.yaml filename="_brand.yml"}
{{< include _examples/websites/brand-fonts/_brand.yml >}}
```

Swap `source: google` for `source: bunny` if you'd prefer Bunny Fonts---a GDPR compliant alternative to Google Fonts.

You can also specify `logo` in **brand.yml** as an alternative way to include a logo in top or side navigation. 
However, if you aren't using **brand.yml** across projects, or formats, it may be easier to specify it directly in `_quarto.yml`.

You can read about all the options available in the Quarto documentation Guide to using **brand.yml**. 
...
To apply your band file on top of a built-in theme, you'll need to specify their order in `theme`:
...
For example, the theme `lux`, explicitly sets a color for headings which doesn't work will with a dark background. 
This explicit value wins over the implicit inheritance from `base` in the brand file.
You can fix it by specifying `headings` explicitly in the brand file:
...
Otherwise, you'd also need to include CSS to import the font.
...
but you can read more in the Quarto documentation on [More about Quarto Themes](https://quarto.org/docs/output-formats/html-themes-more.html).
...
You add SCSS to a document with the `theme` argument:
...
You can layer your custom SCSS with any other elements you might already have:
...
As an example, you might want to override **brand.yml** selection of `primary` for the navigation bar:
...
The names of the options to `bootstrap` correspond to the sections in Quarto SCSS files, with the additional convenience that `defaults` can take a mapping of SCSS variables to value.
...
As an example, Bootstrap includes a button component and gives the example:

```html
<a class="btn btn-primary" href="#" role="button">Link</a>
```

The key information is in `class`. To make a link look like a button, use the usual syntax, but add the classes, not forgetting a `.`, in curly braces:
...
Some content contained within a border using the `primary` color, rounded corners, and some padding.
:::


````markdown
::: {.border .border-primary .rounded .p-2}
Some content contained within a border using the `primary` color and rounded corners, with some padding.
:::
````

`table-striped` example.
...
Find them at <https://github.com/quarto-dev/quarto-cli/blob/main/src/resources/formats/html/bootstrap/dist/scss/_variables.scss>
...
You add an `about` page template to a page by adding the `about` option and specifying a `template`, in this case `trestles`, other options include `jolla`, `solana`, `marquee`, and `broadside`.
...
![Blog listing using `type: default`](){#fig-website-listing-blog}

![People listing using `type: grid`](){#fig-website-listing-people}

![Publication listing using `type:table`](){#fig-website-listing-pubs}
:::

The items in a listing can be other Quarto documents in the project, or you can specify the items using YAML.
...
When you automate publishing, you'll need to consider where you want code execution and rendering to occur: both locally, local code execution with remote rendering, or remote code execution and remote rendering.




Github Most obvious choice


### Local code execution and render
...
From your perspective, you run `quarto render` locally, then push the changes to your repository. Your provider then updates the deployed site.

Two examples of this approach are:

-  Github deploy from `docs/` (as described in @sec-publishing-github)

-  Netlify deploy from a Git provider (as described in @sec-publishing-netlify)

Alternatively, you could set up your own automation that detects changes to `_site` (or `docs/`) and then deploys your content. 

### Local code execution and remote render

In this approach `quarto render` 

This approach requires using `freeze` (@sec-freeze-cache) to store the results of the code execution.
You then check `_freeze` into version control, CI agent just needs Quarto

Don't need to commit `_site/`
...
check only source code, CI agent needs Quarto and R/Python/Julia etc.
...
GitHub Actions are the most common method for setting up continuous integration for publishing Quarto websites.
...
You can then use Actions to automate publishing.

GitHub Actions can be incredibly flexible. 

Use other people's actions, or write custom ones.

Example of action that publishes on changes to `main`. INlcudes code execution.
Means 


A GitHub Action which is a configurable automated process that you set up in your repository using a YAML file to define the specific events, jobs, and steps to be executed.

GitHub Actions are executed on GitHub-hosted runners

Get example and break down into:

- When action should run

- Set up Quarto

- Set up requirements for code execution
