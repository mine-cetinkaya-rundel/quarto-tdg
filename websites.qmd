...
They might all be the same, for example, you might host your source code on GitHub, implement continuous integration with GitHub Actions and publish to GitHub Pages. 
...
This process is illustrated in @fig-website-publish-local. 
We use the term "publish" for what the continuous automation does in the sense that it pushes your content to the host making it "published", not that it calls `quarto publish`.
In this setup, the CI does not need Quarto at all.

![Continuous integration with local code execution and local render ](images/website-publish-local.jpg){#fig-website-publish-local fig-alt="A diagram with four boxes labelled from left to right: Your computer, Repo, CI and Website host. In 'Your computer' the folder `website/` feeds into render and produces `_site/`. The `_site/` folder is moved to 'Repo' via `git push`, then the contents of the `_site/` folder moves from the 'Repo' through 'CI', to the 'Website host'."}
...
Your CI agent then uses the Quarto CLI to render and publish your site (@fig-website-publish-ci).

![](images/website-publish-ci-freeze.png){#fig-website-publish-ci}
...
When your CI runs, it will install your computational environment, install Quarto, and then run a full render and publish (@fig-website-publish-ci-remote).  

![](images/website-publish-ci-remote.png){#fig-website-publish-ci-remote}
...
You simply connect a GitHub repo to your Connect Cloud account, and
