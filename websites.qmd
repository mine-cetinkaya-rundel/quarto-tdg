...
├── _quarto.yml
├── index.qmd             { base }
├── about.qmd             { base }/about.qmd           
├── reports/                       
|    ├── index.qmd         { base }/reports            
|    └── last-month.qmd    { base }/reports/last-month.qmd                        
└── data/                         
...
To link to websites external to yours use the full URL, including the `.html` extension and fragment if applicable:
...
Global: `navbar`, `sidebar`, hybrid

Navigation Items

Other elements - by picture and links to documentation Maybe as a cheatsheet table?

| Element  |              |  Option |
| ---------|--------------|---------|
| [Breadcrumbs](https://quarto.org/docs/websites/website-navigation.html#breadcrumbs) Links above page title that indicate location in sidebar hierarchy  |  ![](images/website-nav-breadcrumbs.png){fig-alt="Text above page title that indicates page in sidebar hierarchy  'Guide > Websites > Website Navigation'"}  | `breadcrumbs`  Under `website` in `_quarto.yml`, or in document header |
...
Add a `_brand.yml` file at the root of your website (i.e. alongside `_quarto.yml`, 
and it will apply to all pages.

Layering **brand.yml** with a built-in theme.
If you want to apply your band file on top of a built-in theme, 
you'll need to specify their order in `theme`:
