...
RStudio provides several features that make Quarto authoring efficient and productive:

**Rendering**: Use the **Render** button or keyboard shortcut (⇧⌘K on macOS, Ctrl+Shift+K on Windows/Linux) to render `.qmd` files to HTML, PDF, Word, or other formats.
...
**Dual editor modes**: RStudio offers both visual and source editing modes. The visual editor provides a WYSIWYM (What You See Is What You Mean) experience with formatting toolbars that make it easy to add formatted text, tables, images, and citations without memorizing markdown syntax.
...
This allows you to test and debug your code incrementally.

**Side-by-side preview**: View your source code and rendered output simultaneously, which works for both HTML and PDF outputs.
...
Beyond the core authoring features, RStudio provides several additional tools that enhance your Quarto workflow:

- **Document outline**: Navigate long documents easily using the outline panel
- **Code snippets**: Insert common code patterns quickly with built-in snippets
- **Project integration**: Manage Quarto projects with multiple files and shared configurations
- **Git integration**: Version control your Quarto documents directly from the IDE
- **Spell checking**: Catch spelling errors in your markdown text
...
2. Install the Quarto extension from the VS Code marketplace
3. Install the Quarto CLI from [quarto.org](https://quarto.org)
...
**TO DO: Decide on whether/how to include visual editor broadly in the book.**
